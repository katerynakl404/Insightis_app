...
- **< 480 px**: composer dropdown labels (`.cl-dropdown-lbl` — "Connections", "Insightis Light (Low)") and the Send button label (`.cl-send-lbl`) collapse to **icon-only**. Glyphs + `aria-label` carry the meaning. Suggestion pills also tighten (h32, smaller padding + 12px font). Same icon-only-on-mobile pattern OpenAI's chat composer uses.

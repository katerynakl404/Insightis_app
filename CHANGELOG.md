...
- **Focus-ring recipe de-duplicated + collapsed to one token.** The inline `0 0 0 2px var(--card),0 0 0 4px var(--focus-ring-brand)` was hand-written on ~25 selectors; the pre-existing `--shadow-focus-brand` tokenised it byte-for-byte but wasn't used everywhere. All focus rings now read a single **`--shadow-focus`** whose halo colour flows through `--focus-ring` (brand by default, overridable per role) — `--shadow-focus-brand` is retired (it rendered identically since `--focus-ring` aliases `--focus-ring-brand`). Added **`--shadow-focus-inset`** for the nested chat-row kebab/pin focus (single inward 2px ring, was inline on 2 sites; also routed through `--focus-ring`). No rendered change — pure DRY; the only intentional inline survivors are `.chat-row` + `.dsf-meta-dl` focus (a `--bg` gap, not `--card`, because they sit on the page surface). Propagated across kit CSS, storybook, `changes/*.md`, `current/SegmentedControl.md`, page + component files.
- **Elevation ladder finished + theme-aware.** Every floating surface was on inline drift; migrated to the canonical ladder. Added **`--shadow-modal`** (deepest rung, `.dlg`). The floating rungs are now **theme-aware** (boosted dark drop + 1px white inset highlight — low-opacity black is invisible on dark). `.sbx-pop`, `.toast` → overlay rung; `.menu`, `.sbx-chat-menu`, `.pop`, `.acpl-list` → menu rung; removed the per-component `.dark .sbx-pop` override (token carries dark). Drift normalised to the rungs (±few % alpha). Inline-by-design (not depth rungs): directional drawer shadows (`.sht-panel`, `.cl-drawer`) + component micro-shadows (`.swt` thumb, `.prov-ic-w`, `.banner-ic`).
- **`--shadow-menu` + `--shadow-overlay-soft` merged → `--shadow-overlay`.** The two were near-identical values that never stack; collapsed into one floating-surface rung (kept the overlay value). Final ladder: `rest → card-hover → lift-hover → overlay → modal`. Propagated across kit CSS, storybook `#shadows`, `changes/{Modal,Toast,Dropdown,Autocomplete,Popover,AccountPopover}.md`.
- **Prod mirror:** add the same `boxShadow` tokens to `@insightis/ui/tailwind.config` — `rim-active` / `rim-hover` / `focus` / `focus-inset` / `overlay` / `modal` (the last two theme-aware) — and replace the arbitrary `shadow-[0_0_0_1px_hsl(var(--border))]` values in SidebarMenuButton, Switch, SegmentedControl, and the account theme toggle.

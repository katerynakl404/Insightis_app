...
- The kit is **three files**: `insightis-preview-kit.html` (markup + storybook JS), **`pages/kit-theme.css`** (all CSS), and **`pages/kit-kit.js`** (shared behaviour). The HTML has *no* `<style>` block — every selector lives in the external stylesheet. **Before editing any CSS, grep the selector in `pages/kit-theme.css` first; do NOT add CSS to the HTML.**
- **`pages/kit-kit.js` owns behaviour that is part of a component's contract** — the tooltip engine (every `[data-tip]`) and menu placement (`.menu.is-up` flip). Every page and the storybook load it in `<head>`. **No page may re-implement anything in it**; a page needing different behaviour is a contract change, made once in `kit-kit.js` for all consumers. Page-specific flows (render functions, seed data, dialogs) stay in the page's own inline `<script>`. This file exists because per-page copies drifted: 7 pages kept a stale tooltip warm-up and one shipped a second, delay-free engine of its own.
- **Colour-token discipline (hard rule, blocks merge):** every colour flows through three layers — Primitives → Semantic → Component-scoped. Component rules read `var(--token)`, never inline `color-mix(...)` / hex / `rgb()` — with ONE sanctioned exception: the tint-scale wash recipe `color-mix(in srgb, var(--<semantic base>) var(--tint-N), transparent)` (both halves are tokens; literal % is still forbidden). Tokenise any expression that repeats — overlay strength (6%, 8%, …) lives in exactly one place: the `--tint-N` steps in the TINT SCALE block. Reuse before inventing. New variants must follow their sibling-family's recipe — Outlined = coloured border + transparent fill, label neutral `Text/Body`, **except Destructive Outlined, whose label is `--fb-red-text` in every state** (agreed 2026-09-18: a neutral label made it read as a plain outline button that happened to be red-edged). A documented exception like that one outranks the family recipe; check `changes/<Component>.md` before "restoring" a variant to its family. Full rule + self-review checklist in [`claude-code/instructions.md`](claude-code/instructions.md) → "Colour-token discipline".
- **The kit is Expected-only.** `insightis-preview-kit.html` shows the target design and nothing else — no "Current (prod)" preview cell, no `Current (prod) · was` column, no `.prod` scope (removed 2026-09-14, see `archive/2026-09-14-kit-current-prod-column/`). Every demo renders on the default `:root` / `.dark` tokens. The prod → Expected story lives in `changes/*.md`, not in the storybook.
...
**Type / icon marks are SQUARE — never stretch one to fit its label.** A coloured mark that carries a short label (`.dsf-file-ic` file-type mark, connector marks, avatars, any future `--mark-bg` square) keeps a 1:1 box. If the label does not fit the box, the label changes (shorten it, map it per type — `xlsx → XLS`) or the box grows on BOTH axes. Never `min-width` + horizontal padding so the mark widens with the text: a row of marks of different widths stops reading as one column. This costs row height — the Files table's 32px mark makes its rows 53px against the Connections table's 45px — and that is accepted; the shape wins over row-height parity (user directive 2026-09-20, after a 24px-tall variable-width version was tried and reverted the same day).

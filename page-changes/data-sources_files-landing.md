...
| 13 | Concept layout variants live on **one page** toggled by a `.segctrl` (Cards / Table). The toggle is a **demo control** — it sits in the **topbar next to the Light/Dark theme toggle, never in the page content**. **Table is the default view** (`viewMode = 'table'`). `viewMode` drives `dsfRender()` → `dsfRenderCards()` (kit `.chat-row`) or `dsfRenderTable()` (kit `table.tbl`, columns: checkbox · Name · Type · Size · Modified · kebab). |
...
| 17 | **File count is shown** in both views. Table: a persistent "N files" (selection count/actions live in the floating bar, so no jump). Cards: "N files · Select all" at rest; during selection it swaps to "N selected" + bulk actions. Count reflects the active filter. |
...
| 19 | **Table bulk actions use a floating bar, not an inline row** (`.dsf-floatbar`, `position:fixed`, bottom). It's centred over the **list**, not the viewport (`dsfPositionFloatbar()` sets `left` to `#dsf-list`'s live centre; recomputed on render + window resize) — because the sidebar/preview offset the table. The inline meta row (count) stays put in table view, so selecting never shifts the table (zero jump). Cards keep their inline meta row. The floatbar shows "N selected · Download · Delete · ✕", visible only when `viewMode==='table'` and ≥1 row is selected. |
...
| 23 | **Tooltips always delay ~300ms on hover** — the tooltip engine's warm-up "instant re-show" was removed (`TIP_DELAY=300` on every hover). Every `data-tip` (sidebar, kebab-less icons, meta ✕, floatbar, preview Download/Close) routes through this one engine. |

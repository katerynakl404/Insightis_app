...
## Popover list states — concept exploration (`chat_page-landing`)

The **Connections** and **Add-file** popovers have three list states — **empty · filled · scrolling** — under exploration on `chat_page-landing.html` via an **on-page concept switcher** (kit `SegmentedControl`, design-review only, sits above the composer). Selecting a state re-renders the **live composer popovers**:

- **Empty** → `.cl-menu-empty` (muted `--ink-secondary` message); the heading and the Manage Connections / Choose File chrome stay.
- **Filled** → the 3 sample rows (this is the page's default — the popovers ship filled).
- **Scrolling** → 12 rows in `.cl-menu-scroll` (`max-height: min(20rem, 60vh)` ⇒ up to ~10 rows when screen height allows, then the row region scrolls; heading + footer pinned).

The switcher is a concept control (not product UI) and is **not** added to `chat-landing.html` — exploration lives on the in-chat page only.

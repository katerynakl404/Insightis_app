...
| **File type icon** | Colored square + extension text | `.dsf-file-ic` coloured square: `.type-csv` → `--brand-tertiary`; `.type-xls`/`.type-xlsx` → `--fb-green`; all via `--mark-bg` custom property. **Locked — the mark is square.** It is the tallest thing in the row, so Files rows stand taller than Connections rows; that is accepted. A 24px-tall variant whose width followed the label was tried on 2026-09-20 to bring the two tables onto one row height and reverted the same day: a four-letter extension does not fit a 24px square at the 10px overline step, and the shape wins. Making it smaller means changing what the label says (a per-type map, `xlsx → XLS`), never the aspect ratio. | Token-based; follows the connector-mark pattern from the Connections tab |
...
> **Where this lives:** the **table** — `.dsf-tbl-wrap`, `table.dsf-tbl` and every `.dsft-*` cell, plus the `.dsf-file-ic` mark and the `.dsft-sortbtn` header — ships in [`pages/kit-theme.css`](../pages/kit-theme.css) and renders in the storybook under **Tables → `#tbl-files`**. It moved there in stages through 2026-09-19/20; before that its desktop half sat in the page `<style>` while its mobile card form was already in the kit, so the storybook could not render the real table and hand-built a lookalike from `table.tbl` plus inline styles. The **page-only** `.dsf-*` rules that remain inline in [`pages/approved/data-sources_files-landing.html`](../pages/approved/data-sources_files-landing.html) are page layout: the drop zone, the filter chip row, the list wrapper, the preview pane and the meta row. The component is composed from kit primitives (`.chat-row`, `.chat-list`, `.cbx`, `.meta-row`, `.meta-row-count`, `.meta-row-actions`, `.menu`, `.mi`, `.danger`, `.link`, `.btn.btn-outline.btn-sm`) plus the page-local `.dsf-*` rules below. To rebuild, copy the kit primitives plus every rule in this section.
...
- `width:2rem; height:2rem; border-radius:.375rem; display:inline-flex; align-items:center; justify-content:center` — **square, locked**.

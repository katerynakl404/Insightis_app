...
|---|---|---|---|
| **Trigger shell** | Native `<select>` in `.field` wrapper — OS-styled, no custom hover/focus | `.sel-trig` — `display: flex; align-items: center`, `gap: .5rem` (8 px), `border: 1px solid var(--border)`, `bg: var(--card)`, `h: 2.25rem` (36 px), `border-radius: .375rem` (6 px), `width: 100%`, `text-align: left`, `cursor: pointer`, `font-family: inherit`, `transition: border-color .12s` | Matches `Input` shell geometry so selectors and text inputs can sit side-by-side consistently |
| **Trigger typography** | OS default | `font-size: .875rem` (14 px), `color: var(--ink)` | Same body size as `Input` text |
| **Trigger padding** | Browser default (varies by OS) | `0 .75rem` (12 px) at `sm`/`md`, `1rem` (16 px) at `lg`, `1.25rem` (20 px) at `xl` | Symmetric, and **identical to `Input`, `TextArea` and `InputGroup`** — every text control shares one left rail, so a form reads as one column. 12 px also matches `.mi`'s horizontal padding, so the selected value sits exactly above the option labels when the menu opens (at 8 px the text jogged 4 px on open/close). Confirmed 2026-09-02. |
...
| **Dropdown item — chosen** | OS-native selection highlight | `.sel-menu .mi[aria-selected="true"]` — `--state-pressed` fill, `--ink-highlight`, weight 500; hover keeps the fill | The system chosen recipe, same as the sidebar active row. Reopening the menu previously showed no trace of the current value: the trigger held it, the list did not. Hover keeping the fill matters — otherwise hovering the chosen row makes it read as unchosen. |

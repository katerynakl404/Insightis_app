...
| Section header (collapse trigger) | — | — | the **entire `.sbx-sect-head` row** is the collapse/expand target — `.sbx-sect-label-wrap` stretches `flex:1` from the left padding to just before the `See all` link, so clicking anywhere on the row (label, chevron, or the empty horizontal space around them) toggles `aria-expanded`. The `See all` link sits as a sibling and stays an independent click target — clicking it navigates, never collapses. | full-row hit target (≥ 213 × 24 px at the default 288 px sidebar width) makes collapse easy to discover; "See all" remains its own escape hatch |
...
|---|---|---|---|---|
| Default | h7 `SidebarMenuSubButton` with `text-content-secondary` and an indented left-rail border | — | flat h28, padding 0 8, text 13px `Text/Secondary`, single line. Label fades right via a **persistent** 36px linear gradient from `transparent → row bg` — no ellipsis dots, label is never re-revealed on hover. **No hover tooltip** on truncated titles (prod wired [`TruncatedTitleTooltip`](TruncatedTitleTooltip.md) here; Expected drops the wiring — the fade alone signals truncation, and a hover tooltip duplicates the information without adding interaction value). | quieter than `text-overflow:ellipsis`; gradient's end-color is parametrized via `--chat-fade` so it tracks any row state. Token stays `Text/Secondary`; the new Slate-550 primitive (see [colors](colors.md)) is what clears AA on hover. |
...
- Hit target ≥ 24×24 on every interactive element (nav row 32, chat row 28 × full row width, kebab 24×24, user row ≥ 28).
...
|---|---|---|---|---|
| **≥ 768 px** (desktop + tablet) | In-flow fixed-width column | `SIDEBAR_WIDTH` `14.75rem` (or page override — chat-landing uses `16rem`) | In-sidebar collapse trigger (`.sbx-collapse`) | Visible by default. Trigger toggles **in place** to `SIDEBAR_WIDTH_ICON` `3.563rem` (icon-only) and back. No overlay. Icon-rail (`.sbx.is-collapsed`) rules are wrapped in `@media (min-width: 768px)` so the class has no visual effect in the mobile drawer. |
| **< 768 px** (mobile — matches prod's `useIsMobile` hook) | Hidden by default → overlay on demand | `18rem` when open; otherwise not rendered | **Topbar hamburger** (`.cl-burger`) | Sidebar is `display:none` until the user taps the hamburger. Tap → sidebar mounts as a **position:fixed overlay** at `18rem` with a semi-opaque backdrop dimming the main column. The in-sidebar trigger acts as a **close** button (same as backdrop). |
...
- The in-sidebar trigger (`.sbx-collapse`) is one onclick that branches on viewport: ≥ 768 px → toggle `.sbx.is-collapsed`; < 768 px → remove `.side-open` (close overlay).

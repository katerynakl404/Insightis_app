...
|---|---|---|---|---|
| Default | h7 `SidebarMenuSubButton` with `text-content-secondary` and an indented left-rail border | — | flat h30, padding 0 8, text 13px `Text/Secondary`, single line. Label fades right via a **persistent** 36px linear gradient from `transparent → row bg` — no ellipsis dots, label is never re-revealed on hover. **No hover tooltip** on truncated titles (prod wired [`TruncatedTitleTooltip`](TruncatedTitleTooltip.md) here; Expected drops the wiring — the fade alone signals truncation, and a hover tooltip duplicates the information without adding interaction value). | quieter than `text-overflow:ellipsis`; gradient's end-color is parametrized via `--chat-fade` so it tracks any row state. Token stays `Text/Secondary`; the new Slate-550 primitive (see [colors](colors.md)) is what clears AA on hover. |

...
| Aspect | Current (prod) · was | Expected · became | Specification |
|---|---|---|---|
...
| State / part | Current (prod) · was | Expected · became | Specification |
|---|---|---|---|
| Nav row count | 4–5 menu rows (Sources, Metrics, Favorites, Chats) | **2 rows** — Sources, Metrics | other prod groups (Favorites, Chats) are removed from the nav and replaced by the Pinned/Recent chat sections below |
...
| Active (`data-[active=true]`) | faint accent-tinted bg (≈ `bg-accent/6 %`) + text & icon `accent` + `font-medium` + **left-edge accent rail** (`::before`, 3 px × nearly full row height, rounded, `bg-accent`, `left:0`) | **color-only** — icon & text `Brand/Primary`, **no background, no rail** | quieter; active reads via color, not via two surface cues |
...
| **Active** (`data-[state=active]`) | **full-row** `bg-background` + text `accent` | **color-only** — text `Brand/Primary`, **no background**; gradient stays at default `Surface/Card` fade | preserves the row's quiet aesthetic; active reads via color |
...
| **User row — click** | no menu | **Opens the Account popover** above the row. Two labelled sections (`Account`, `Support`), each containing `SelectMenuItem` rows (h24, gap 6 px, 12 px Lucide icon + 12 px label, hover bg `State/Hover`). Segmented theme switcher (sun / moon / monitor-speaker) at the bottom uses the existing `Tabs`-style pattern. Sign Out is a plain `SelectMenuItem` (neutral text, log-out icon). | reuses [`Popover`](Popover.md) `.sbx-pop` shell — single class drives both popovers |
| **Tokens row** | plain `<div>` block | now a **tertiary-styled `<button>`** (transparent bg, `--brand-primary @ 6%` hover, `--brand-primary @ 8%` pressed, `--shadow-focus-brand` focus). On click opens the **Subscription tokens popover** above the row with: shield-check plan badge + plan name, two metered rows (`Subscription Tokens` `Brand/Tertiary` progress, `Purchased Credits` `Feedback/Green` progress, 4 px bar), and two CTAs — `Buy Credits` (`Button primary`) and `Upgrade Plan` (`Button outline`), both h36 `rounded-full`. | reuses [`Popover`](Popover.md) `.sbx-pop` shell + existing `Button` primary/outline variants — no new component styles for the buttons |

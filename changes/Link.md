...
| Focus (keyboard) *(new)* | ⚠ browser-default ring | — | `box-shadow: 0 0 0 2px Surface/Card, 0 0 0 4px --focus-ring` + `border-radius: .125rem` | same recipe used by Button / IconButton / nav rows / `.sbx-pop-theme-btn` — one ring shape across the system |
...
`--ink-highlight` (default colour, theme-adaptive — resolves to `--brand-primary` Brand-600 `#07807E` light / `--tertiary-400` `#2EBEC4` dark) · `--ink-inactive` (disabled colour) · `--focus-ring` (focus ring) · `--card` (focus-ring gap). All resolve via existing semantic aliases — no new tokens.

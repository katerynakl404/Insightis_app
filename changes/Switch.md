...
| Off (default) | `background` + 1 px border `Stroke/Border`, thumb `content-light` (page-coloured track, barely visible) | — | filled `--switch-off-bg` = Slate-300 `#CBD5E1` light / Grey-700 `#2A2834` dark; thumb `Content/On_Solid` with `box-shadow: var(--shadow-thumb)` (drop-shadow + faint 0.5 px hairline → perceivable thumb edge against the medium-grey track). **The value is now a token** — `--shadow-thumb` — because [StepSlider](StepSlider.md) uses the identical knob lift and the two controls sit side by side in the composer model popover; the rendered value is unchanged from the earlier literal | Component-scoped token so light/dark/prod each cleanly set their own off-bg without cascade collisions. No outline, no border — cleaner read than the earlier outlined-stroke draft. |

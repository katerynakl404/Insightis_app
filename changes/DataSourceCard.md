...
> **New component — no prod equivalent.** The Connections catalog did not ship this card on prod, so there is nothing to diff against. The Expected spec below is the agreed contract, derived from the live mockup CSS/JS in `pages/approved/data-sources_connections-landing.html` (the `.ds-card` rules + `dsCardHtml()` / `dsMarkWrap()` builders).

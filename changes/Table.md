...
| **No-built-in row** | `tr.mx-no-builtin-row` | `cursor:default !important`; non-interactive (`:hover td,:active td{background:inherit !important}`). Cell `td{padding:.625rem 1rem;padding-left:var(--mx-prov-indent);border-bottom:1px solid var(--border);min-height:2.875rem}`. |

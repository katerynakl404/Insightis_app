...
| **Nested row** | `tr[data-nested]` inside `table.tbl` | A row that BELONGS to the row above it — a metric under its provider, a schema under its connection. `td{padding:.5rem 1rem}`: 8px instead of the base row's 10px, and that is the **only** difference. A child row set at the same height as its parent reads as its sibling; the tighter rhythm is what says it is not. Anything louder — a tint, a smaller type size — makes a child look like a different *kind* of object rather than the same object one level down. Horizontal padding is unchanged, and the first cell's indent stays with whoever owns the parent row (it depends on what is in the parent's first cell — a chevron, a logo, both — which the table cannot know). Previously this existed only as `.mx-metric-child td`, a local override on the metrics table, so every other place that nests rows either invented its own number or did not tighten at all. Mirrors the `nested` prop on `TableRow` in `@devart/ui-react`. |
...
| **Metric child row** | `tr.mx-metric-child` | 6-cell row: toggle · name · alias · description · badge · actions. Inherits `cursor:pointer` from the shell. Carries `data-nested`, so its 8px vertical padding comes from the shared `tr[data-nested]` rule (see "Nested row" above) rather than from here — this entry used to claim `.4375rem` while the CSS said `.5rem`, which is what a locally-owned number costs. What stays local: `td{height:2.875rem;box-sizing:border-box}` — fixed row height so every row is identical regardless of content (cells use `vertical-align:middle`). First cell `td:first-child{padding-left:var(--mx-prov-indent)}` aligns the toggle to the shared indent line. Hover/selected/pressed follow canonical base `.tbl` rules (`--tbl-row-hover` / `--tbl-row-pressed`); `.mx-tbl` only adds `cursor:pointer`. |

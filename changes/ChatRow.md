...
| Hover | ⚠ pending | — | bg `State/Hover` (`--state-hover`); border `--card-border-hover` (`color-mix(in srgb, var(--brand-primary) 25%, transparent)`); box-shadow lifts to `--shadow-card-hover` (`0 2px 6px -1px rgba(15,23,42,.08), 0 1px 2px 0 rgba(15,23,42,.04)`); the row kebab fades in (opacity 0 → 1). | same fade recipe as `.sbx-chat-more`; reveal triggers on `:hover`, explicit `.s-hover`, `:focus-within`, kebab `:focus-visible`, and kebab `aria-expanded="true"`. **Menu-open keeps hover:** `.chat-row:has([data-kbp][aria-expanded="true"])` is matched by the same hover rule so the row holds its hover bg/border/shadow while its menu is open. |
...
| Kebab (more) | ⚠ pending | — | The shared **row kebab**: a kit IconButton at the row step (`.iconbtn.iconbtn-tertiary.iconbtn-2xs`) carrying `data-kbp`. Size, radius, glyph size, colour, hover, pressed and focus all come from [IconButton](IconButton.md) — this row adds nothing of its own except the reveal trigger. Hidden at rest; revealed by row `:hover`/`.s-hover`/`:focus-within`, by the kebab's own `:focus-visible`, and by `aria-expanded="true"`. **Changed 2026-09-19:** was `.chat-row-more`, a hand-built copy of IconButton that re-declared size and radius and replaced the neutral hover with a brand tint. The tint resolved to the identical value on light theme and only ever differed on dark, where it left this kebab as the single brand-tinted hover in a neutral row. The neutral `--state-hover` overlay stacks on top of the row's own `--state-hover`, so the button still reads as a distinct surface on a hovered row — which is all the tint was for. Focus is now the canonical outset halo (`--shadow-focus`); the inset ring was only needed for the gradient-masked sidebar row. | one component across Chats, Files, Metrics and Connections — see the "Row kebab — ONE component" block in `kit-theme.css`. |
| Open menu | ⚠ pending | — | The kebab and its menu sit in `.kbp-wrap` (the positioning context is the trigger's own box, so the 8px `--menu-gap` is measured from the button, not from the row). The kebab anchors the shared `.menu.kbp-menu` dropdown, shown by the kit's `[data-kbp][aria-expanded="true"] ~ .kbp-menu` rule; the row only overrides its horizontal anchor (`.chat-row .kbp-menu`). Parent list flips to `overflow:visible` while open so the menu can escape its row. Shell reuses kit `.menu` / `.mi`; items: Pin/Unpin (label reflects current state), Rename, Delete (`.mi.danger`). Outside click + Esc close it; only one menu open at a time. | While the menu is open the trigger holds its pressed surface and the row holds its hover — the pointer is down in the dropdown, not on the row, and a trigger that disappears under its own open menu is an impossible state. |
...
<div class="chat-row">                       <!-- add .is-selected / .s-selected when chosen -->
  ...
  <button class="iconbtn iconbtn-tertiary iconbtn-2xs" data-kbp="1" type="button" aria-label="More actions" aria-haspopup="menu" aria-expanded="false"> <!-- kebab; 3-dot vertical <svg> -->
    <svg viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="6" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="18" r="2"/></svg>
  </button>
  <div class="menu kbp-menu">…</div>     <!-- context menu; shown via aria-expanded sibling rule -->
</div>
```
...
`--card` (rest bg) · `--border` (45% mix → rest border) · `--ink-body` (default text, kebab hover colour) · `--state-hover` (hover bg) · `--state-pressed` (selected bg) · `--card-border-hover` (hover border = brand 25% mix) · `--brand-primary` (pinned marker colour, selected border, 8% mix for pin-hover) · `--ink-secondary` (timestamp) · `--focus-ring` (focus ring — outer ring of row, inset ring of pin/kebab) · `--bg` (**inner** ring of row focus-visible — not `--card`) · `--shadow-rest` (rest + press shadow) · `--shadow-card-hover` (hover lift shadow) · `--motion-slow`/`--motion-fast`/`--motion-base`/`--motion-ease-out`/`--motion-ease` (transitions) · `.cbx.on` (checked → `--brand-primary` + `--content-on-solid`) · `.mi.danger` (`--fb-red-text`). No new tokens introduced — every state composes from the existing palette.

## `.is-preview` — preview-open row (lifted from page CSS 2026-09-19)

The row whose file is showing in the side preview. The **name** goes `--brand-primary` while the row surface stays at rest, so "being previewed" reads differently from "selected" (which tints the surface). The table layout takes the same state: `.dsf-tbl tbody tr.is-preview .dsft-fname` for the name, `--tbl-row-pressed` on the cells.

Was `data-sources_files-landing.html`'s own rule — a state of a kit row described on a page, so the storybook never showed it.

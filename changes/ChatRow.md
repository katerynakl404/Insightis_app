...
|---|---|---|---|
| Default (unpinned) | ⚠ pending | transparent bg, text `Text/Body`, `.875rem · 500`, height 40 px, radius `md 6 px`. Checkbox slot reserved but **hidden** (opacity 0). No pin marker, kebab hidden. | layout: `flex; gap:.625rem; padding:0 .5rem; align-items:center` |
| Hover | ⚠ pending | bg `State/Hover` (`--state-hover`); checkbox + kebab fade in (opacity 0 → 1, 120 ms). | same fade recipe as `.sbx-chat-more`; works on `:hover`, `:focus-within`, and explicit `.s-hover`. |

...
| Chat title kebab / message copy | `.cp-title-btn`, `.cp-msg-action` (+ `.iconbtn.iconbtn-tertiary.iconbtn-2xs`) | **Added 2026-09-19:** both chat pages used to hand-roll a 24px button in their own `<style>` — same recipe as the `2xs` step, but the title kebab had already drifted to a 6px radius and both sat a shade lighter at rest. They now compose the step. The only rule left is the title kebab's held-open `[aria-expanded="true"]`, in `kit-theme.css`: it takes the same `--state-pressed` surface as the row-kebab contract, which cannot be reused directly because that one starts at `opacity:0` and reveals on row hover — a page-title trigger has no row to hover. |

...
| **Opening direction** | always downward — a trigger near the viewport bottom clipped the menu, putting Delete out of reach | — | `.menu.is-up{top:auto; bottom:calc(100% + var(--menu-gap, 8px))}` — applied on open by the shared placement helper | The trigger gap is **8px everywhere**, declared once as `--menu-gap`; both directions read that one token, so adding an anchored variant is one declaration rather than two mirrored offsets. **Changed 2026-09-19:** it was `2px` on `.kbp-menu`, `-.25rem` inside `.chat-row` and `4px` on `.sbx-chat-menu` — three answers to one relationship, and the per-surface overrides are gone. The gap is measured from the **trigger**, which is why a row kebab and its menu sit inside `.kbp-wrap`: without it the menu measures from whatever is positioned above (a 52px table cell) and an 8px gap renders as 22px. |
...
Items are `<div class="mi">` (the kit ships them as `div`s; `.mi` carries `border:none;background:transparent;text-align:left;width:100%` so a `<button>` works identically). `.menu` defaults to `width:max-content` clamped 140–320 px (variants `.sbx-chat-menu`, `.kbp-menu`, `.ds-conn-menu` inherit). **Menu size system (opt-in fixed widths):** `.menu.is-sm` = 14rem · `.menu.is-md` = 18rem · `.menu.is-lg` = 22rem — for menus that should read as a roomy, consistent popover rather than hug their content (the composer Attach/Connections/Models menus use a fixed **16rem** — one width across all states — at the default 4px padding). Optional WIP stub `.menu-wip` prepends inside any not-yet-redesigned menu. A menu may also carry **non-item settings rows** — a label plus its own control, not an activatable `.mi` and therefore with **no hover surface** — on the same 6px/12px padding rail so every label shares one left edge; the composer Models popover is the first consumer (`.cl-model-opt`, see [../page-changes/chat-landing.md](../page-changes/chat-landing.md) and [StepSlider.md](StepSlider.md)).

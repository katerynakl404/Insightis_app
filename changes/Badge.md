...
**Error glyph — 2026-09-25.** The error mark is a circle with an **X** (`circle-x`), not an exclamation. That was already the system glyph — the error toast draws it — but the Connections Last-check pill and StatusView error drew a circle with an exclamation, so three different marks were in play for two meanings. Triangle stays reserved for warnings: destructive confirms and warning toasts.
